...
async function DeployContract(instance: Web3Instance, abi: Interface, byteCode: BytesLike | { object: string }, ...args: any[]) {
    ...
    const estimatedGas = (await instance.provider.estimateGas(deployTransaction)) * 2n;
    console.log(`Balance required: ${formatEther(feeData.gasPrice! * estimatedGas)} ETH`)

    const balance = await instance.provider.getBalance(instance.signer.address, "latest");
